        ...
        try {
            ...
            if (cantidadEntregada > 0) {
                try {
                    ...
                    // Este bloque nunca debería ejecutarse si la excepción se maneja correctamente
                    e1.printStackTrace();
                }
            } else {
                cantidadEntregada = 0; // Asegúrate de que no se entregue una cantidad negativa
            }
        }

        return elSurtidor.venderGasolina(nombreTipoGasolina, cantidadEntregada);
    }

    public int venderGasolinaPorPrecio(String nombreTipoGasolina, int valorSolicitado, int numeroSurtidor) {
        TipoGasolina tipo = tiposGasolina.get(nombreTipoGasolina);
        double precioPorGalon = tipo.getPrecioPorGalon();
        if (precioPorGalon > 0) {
            double cantidadSolicitada = valorSolicitado / precioPorGalon;
            return venderGasolinaPorCantidad(nombreTipoGasolina, cantidadSolicitada, numeroSurtidor);
        } else {
            return 0; // Prevenir división por cero si el precio por galón es cero o negativo
        }
    }
